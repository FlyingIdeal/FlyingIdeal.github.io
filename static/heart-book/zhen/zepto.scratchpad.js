...
;(function ($) {
  ...
  ScratchPad.prototype = {
    _generate: function () {
      ...
      this.$img = $('<img src=""/>').attr('crossOrigin', '').css({position: 'absolute', width: '100%', height: '100%', opacity: '0 !important'});
      ...
      // Apepnd items
      this.$el.append(this.$scratchpad);//.append(this.$img);
    ...
    reset: function () {
      ...
      // Set bg.
      if (this.options.bg) {
        if (this.options.bg.charAt(0) === '#' || this.options.bg.charAt(0) === 'r') {
      ...
      if (this.options.fg) {
        if (this.options.fg.charAt(0) === '#') {
          ...
          /*beight--1--*/
          var $img = $(new Image()).attr('src', this.options.fg);
  ...
  $.fn.wScratchPad.defaults = {
    ...
    fg          : '',  // Foreground (image path or hex color).
