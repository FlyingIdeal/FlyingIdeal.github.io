$(function() {
	...
	for(var i1=1;i1<=5;i1++){
		imgData1.push({name: "d0"+String(i1), path: "img/d0"+String(i1)+".jpg"});
	}
	//imgData1.push({name: "d0"+String(i1), path: "img/d06.png"});
	...
	function initMusic(){
		...
		var isMusicOn = true;
		var audio = new Audio('music/bgmusic.mp3');
		audio.load();
		audio.loop = true;
	...
	function initPartA(id){
	    ...
	    var partAniSp = new LSprite();
	    partLayer.addChild(partAniSp);
	    partAniSp.addChild(partAniLayer);

	    
	    partAniLayer.addEventListener(LEvent.COMPLETE,function(e){
	    	partAniLayer.stop();
	    	partAniSp.addChild(partScene);	    	
	    	LTweenLite.to(partScene,0.5,{alpha:1});
	    	setStars(id-1);	    	
	    	partAniSp.removeChild(partAniLayer);
	    	partAniLayer = null;
	    });
